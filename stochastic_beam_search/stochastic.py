...
def Stochastic_Beam_search(routine,beam_width):
    ancestor = []
    cost_list = []
    rand_arr = []
    max_cost = -1
    ...
    while (1):
        ...
        for i in range(0, len(ancestor)):
            ...
            while not p.empty():
                ...
                cost_list.append((ocost, obj))
                counter += 1
        tuplettt = q.get()
        best_child = tuplettt[2]
        if best_cost < best_child.get_cost():
            return best
        q.empty()
        for m in range(0,len(cost_list)):
            tuplett = cost_list[i]
            tcost = tuplett[0]
            if(tcost > max_cost):
                max_cost = tcost
        for n in range(0,len(cost_list)):
            tup = cost_list[i]
            cost_dup = tup[0]
            iter = max_cost + 1 - cost_dup
            obj_dup = tup[1]
            for p in range(0,iter):
                cost_list.append((cost_dup,obj_dup))
        counter = 0
        ancestor = []
        k = 0
        pass_code = -1
        ...
        while (k < beam_width):
            idx = random.randint(0,len(cost_list)-1)
            if len(rand_arr) != 0 and idx in rand_arr:
                pass_code = 0
            else:
                pass_code = 1
                rand_arr.append(idx)
            if pass_code == 1:
                sel_tuple = cost_list[idx]
                obj_cost = sel_tuple[0]
                obj_child = sel_tuple[1]
                if len(ancestor) != 0:
                    for x in range(0,len(ancestor)):
                        if obj_child != ancestor[x]:
                            ancestor.append(obj_child)
                            k += 1
...
def main():
    ...
    for i in range(0,itnum):
        ...
        final_routine = Stochastic_Beam_search(routine,beam_width)
